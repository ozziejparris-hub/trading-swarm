...
**Status:** Migration IN PROGRESS — **Stage 0 COMPLETE**, **Stage 1 COMPLETE**, **Stage 2 SHIPPED 2026-07-13** (first-repo `3371a1a`), Stages 3-5 not started. This is the first stage that touches the previously-frozen production write path — see §5 for what shipped and the post-run verification checklist for the 2026-07-14 06:00 maintenance cycle. Stage 0: 0a redefined+passing+write-time-corrected, 0b resolved `W_beh=0`, 0c done, 0d harness OBSERVE-mode invariants landed. Stage 1: pure formula built, 339,663 test assertions green including a 61,248-point zero-diff equivalence proof, live-data validation at 99.87% match, `elo_shadow` table + delta report reviewed. Stage 2: `apply_full_elo_modifiers.py` migrated onto the canonical formula + atomic write helper, proven output-neutral via a full-snapshot dry-run (zero diffs across all 157,527 traders after fixing a rounding bug the zero-tolerance check caught).
...
**ADDENDUM 2026-07-13 — §2.2 was ALSO revised, missed by the pass above.** This revision list was written same-day as the fix and named only §2.1/§2.4/§2.5/§4.1/§5; §2.2's justification prose (all three of its sub-points, (a)/(b)/(c)) still stated the pre-correction, un-scaled-bonus numbers for eight days, discovered 2026-07-13 while deriving a Stage 1 property test directly from the formula (the test author trusted the formula over §2.2's prose specifically because it looked suspicious — see the overhang ledger's lesson entry on this). §2.2 is now corrected in place; see its own inline note for the specifics. **Lesson:** when correcting a claim in a document, grep for every place that claim's *numbers* appear, not just the sections remembered as needing the fix — a correction pass is itself a claim that can go stale exactly like the thing it corrected.
...
**(a) No high-ELO inflation.** The behavioral delta is bounded at `damp × w_beh × (0.4 × base + 100)` — both halves (multiplier and bonus) now share the same `w_beh`, per the 2026-07-06 Correction (§2.1). At the design's reference weight `w_beh=0.5` this is `damp × (0.20 × base + 50)`: **+350 at base 1,500; +424 at base 2,400** (where damp=0.8 bites) — verified directly against `compute_comprehensive_elo` with `beh_mult=1.40, bonus=100` (the joint extreme), matching worked example X's `gain_beh` exactly. Compare Writer A's theoretical multiplicative form at base 2,400 with beh 1.3: **+720 from the multiplier alone, before the bonus and before P&L compounds on top of it**. Dampening applies to the *total* gain, so behavioral and P&L cannot jointly escape it. Both caps survive: Writer B's hard 3,500 and Writer A's resolved-indexed soft cap (which binds meaningfully — 206 of the current Sunday population sit exactly on it).

**(b) Behavioral actually incorporated.** A trader with beh 1.2 / bonus +40 at base 2,400 gains **+208** over today's pnl-only value (worked example H below, `gain_beh=260` damped by 0.8 → matches the design table's own `U−B` column) — most of a tier width where it matters for ELITE gating, which is precisely the STR-002 complaint being fixed.

**(c) Sane population values.** Population behavioral mean is 0.999 — the multiplier term is near-zero-mean by construction, so the population mean shift ≈ mean(bonus × w_beh × damp) among resolved≥10 traders. This is measured exactly in the shadow stage before anything ships (§5, Stage 1); with timing excluded (§3.4), the bonus loses its flat +10-for-everyone offset, further centering it.

**[CORRECTED 2026-07-13 — see the Correction section at the top of this document.]** This entire §2.2 previously stated the pre-correction, un-scaled-bonus numbers throughout (a), (b), *and* (c) — the 2026-07-06 Correction's revision list named only §2.1/§2.4/§2.5/§4.1/§5 as updated, and missed this section entirely. All three sub-points above used `bonus × 1.0` (the old independent `W_bonus`) instead of `bonus × w_beh`; every number here is now re-derived directly from the corrected formula and cross-checked against the (already-corrected) §2.4 worked examples, not re-typed from the original draft.
...
- **SHIPPED 2026-07-13 (first-repo `3371a1a`).** Replaced `apply_full_elo_modifiers.py` formula internals with `compute_comprehensive_elo(w_beh=0, apply_soft_cap=False, apply_floor=False)` + the new `monitoring.database.write_elo_result()` atomic full-column-set write helper. At these settings the canonical formula **is algebraically Writer B, term for term** (re-confirmed in the Correction section above) — so this cutover is provably value-neutral.
- **Verification (done):** full dry-run on a fresh backup, copied to two isolated DBs — old code (git HEAD) against one, new code against the other, diffed all 157,527 traders on every written column, **zero tolerance, not a rounding epsilon**. First pass caught a real bug: `write_elo_result()` unconditionally rounded `base_category_elo` to 6 decimals, but the old code only rounds on the backfill branch (the preserve branch reassigns the column's existing value untouched) — 27,597 rows silently truncated, an error small enough (~5e-7) that a looser epsilon-based diff would likely have missed it. Fixed by moving the rounding decision to the caller. Re-ran clean: **comprehensive_elo, pnl_modifier, base_category_elo, behavioral_modifier, advanced_modifier, kelly_alignment_score, patience_score, timing_score all zero diffs** across all 157,527 traders. The two intentional diffs both confirmed exactly as scoped: (i) `elo_last_updated` switched to canonical format for exactly the 30,999 written rows, nothing else; (ii) component columns value-identical (confirmed, not just assumed). Full test suite (339,663 assertions) green, no regression.
- **Rollback:** git revert; values were never different.
- **Runs on the next scheduled maintenance cycle (06:00 2026-07-14)** — this commit made no manual write to the live DB. Backup taken beforehand: `backups/markets_20260713_203618.db`.
- **POST-RUN VERIFICATION CHECKLIST (for the 2026-07-14 06:00 run, or whenever the next `apply_full_elo_modifiers.py` invocation lands):**
  1. `comprehensive_elo` values UNCHANGED vs pre-migration — sample a handful of addresses and compare today's value against `backups/markets_20260713_203618.db`'s value for the same address (should match to the same precision Writer B always stored, i.e. 4 decimals).
  2. `elo_last_updated` is now canonical (space-separated) format for every row the run touches, not T-separated.
  3. Stage 0d's write-atomicity baseline (270 traders with non-NULL `comprehensive_elo` but a NULL component column) should start **dropping toward 0** as each Writer-B-eligible trader gets rewritten atomically — check `audit_invariants.py`'s `[0d/OBSERVE] comprehensive_elo non-NULL with a NULL component column` count against today's 270 baseline.
  4. Re-run `audit_invariants.py` and confirm no NEW Tier-0/OBSERVE drift beyond what's explained by this migration (range/soft-cap/behavioral-materialization counts should be stable or moving in the expected direction, not surprising).
