...
**RISK/EFFORT:** Medium. Doc recommends (not yet implemented): (1) a bounded one-time backfill script hitting Gamma's single-market endpoint directly per row (not the broken list-pagination path) — ~194K requests, single bounded run, highest leverage given the 85% trader overlap; (2) fix the `run_recent_overdue_pass`/`run_stale_clob_pass` handoff so markets that age past 7 days unresolved don't become permanently orphaned (low current urgency per the static-backlog finding, but the structural bug is real and could recur); (3) lower-priority — migrate `fast_resolution_check.py` to Gamma's `/markets/keyset` pagination to recover Pass 1's intended reach.
**FROZEN-AREA?** No.
