...
- **NEW GATE:** `stale_elo_orphans` = COUNT(BUY trades, no matching position, trader `is_flagged=1 AND research_excluded=0`, trade older than 24h). **Current: 4. Threshold: ≤20, sustained across 2-3 samples spread over a day** (per this entry's own multi-sample lesson). Confirmatory sample still owed — 2026-07-12's readings spanned the O-14 outage, not a clean cycle.
...
- **UNBLOCKS:** Stage 0a no longer gates on the old metric. Stage 0c and 0b (neither of which depended on this gate) can proceed now; Stage 0a re-evaluates against the new gate once a clean confirmatory sample lands.
