...
**STAGE 0 COMPLETE 2026-07-12 (0a redefined+passing, 0b resolved at W_beh=0, 0c deleted).** **Consequence for the migration risk profile:** with `W_beh=0`, Stage 2's canonical formula (design doc §2) is **byte-identical to today's live Writer B** — the behavioral term zeroes out entirely, so Stage 2 isn't introducing a new formula with a new weight, it's re-plumbing the *exact current output* through the single-writer architecture. The migration collapses to a pure plumbing unification with zero value changes at this stage, which is a materially lower-risk shape than what the design doc originally scoped (a live behavioral-weight decision baked into Stage 2 itself). Stage 1 (shadow computation) is next: `comprehensive_elo_formula.py` as a pure function + golden tests + the exact-equivalence test (`compute(w_beh=0, apply_soft_cap=False, apply_floor=False) == WriterB_formula()` across the input grid, asserting zero diffs) — the test that would have caught the original Stage-2 bonus-leak bug (see O-7's 2026-07-06 entry) before it was banked.
