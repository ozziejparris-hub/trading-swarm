...
**CLOSED 2026-07-01 — spot-check complete, read-only.** Reproduced the exact 724-row segment (join-scope count came back 298,633, matching within normal daily drift) and sampled 25 of them (~3.5%) via direct `GET gamma-api.polymarket.com/markets/{api_id}` calls, read-only. Result: **0/25 dead/stub** — every one has a real title matching a real-world question and a clean Gamma record. **19/25 (76%) are already `umaResolutionStatus: resolved` on Gamma, with a valid winning outcome in `outcomePrices`, but still sitting at `resolved = 0` locally** (e.g. "Will Solana reach $400 before 2026?" → resolved NO on Gamma, `last_checked` frozen at 2025-12-11 in our DB). **6/25 (24%) are genuinely still open** (`closed: false`, e.g. 2026 governor-race markets) — correctly not yet resolvable by anything, not a gap. Extrapolated, an estimated ~550 of the 724 are sitting resolved-on-Polymarket / stuck-unresolved-locally right now.

Two follow-on findings, both bearing directly on the removal decision:

1. **Pass 1 cannot reach any of the resolved 724, confirmed two ways.** First, none of the 25 sampled markets — resolved or not — have an `endDate` field on Gamma's side at all (full-field dump on one confirmed: no `endDate` key present, only `startDate`/`closedTime`/`umaEndDate`). Under pass 1's `order=endDate&ascending=false` sort, no-`endDate` rows sort last among closed markets, so a 50K (or even a much smaller) window never surfaces them — this isn't a "just inside/outside the window" edge case, they're categorically excluded by the sort key itself. Second, and unexpected: **live-tested `GET gamma-api.polymarket.com/markets?closed=true&order=endDate&ascending=false`, and Gamma now hard-errors with HTTP 422 `"offset too large, use /markets/keyset for deeper pagination"` starting around `offset=2000-2200`** — nowhere near the 50,000 the code is written to respect (`fast_resolution_check.py:132-134`). `fetch_all_resolved_markets`'s status-code check (`if response.status_code != 200: break`) handles this gracefully (no crash), but it means **pass 1's actual daily reach today is ~2,000 closed markets, not 50,000** — a preexisting, Gamma-side regression unrelated to O-13, but worth its own follow-up ticket since it affects pass 1's coverage broadly, not just this 724-row segment.

2. **`check_market_resolutions` is not a working backstop for this segment either — it's mechanistically broken, which is *why* the DB forensics in checklist item 2 show zero resolutions since 2025-12-11.** Traced the actual call path: `get_unresolved_markets()` returns the local `market_id` column, which for condition-id-keyed rows (including all of these 724, where `market_id == condition_id`) is a `0x...` hex string; `trader_analyzer.py` calls `self.polymarket.get_market(market['market_id'])` → `GET gamma-api.polymarket.com/markets/{market_id}`. **Live-confirmed Gamma rejects hex condition-ids at this endpoint with HTTP 422 `"id is invalid"`** — so `get_market_details()` returns `None` before any resolution logic runs, for every hex-keyed market. Separately, even for markets reachable by numeric `api_id`, `check_market_resolutions`'s outcome-parsing (`trader_analyzer.py:186-196`, `outcome.get('payoutNumerator', 0)`) assumes `outcomes` is a list of dicts — but Gamma's current `/markets/{id}` response returns `outcomes` as a plain list of strings (`["Yes","No"]`), with winner data in the separate `outcomePrices` list instead. `str.get(...)` raises `AttributeError`, silently swallowed by the function's blanket `except Exception`, counted as an api_failure, and skipped. (`fast_resolution_check.py`'s `extract_winner` already reads `outcomePrices` correctly and is unaffected — this bug is specific to `check_market_resolutions`.) Net: `check_market_resolutions` provides **zero effective resolution coverage today, for any market**, not just this 724-row segment — matching the empirical zero-since-Dec-11 finding exactly.

**Verdict on the open item: not a coverage risk, and removal is fully clear.** `check_market_resolutions` is not currently closing this gap — there is no working collection path for the 724 to lose. The ~550-market resolved-but-stuck backlog is real and predates this fix, but it's a pre-existing, independent data-quality issue (broken outcome-parsing + hex-id lookup + Gamma's new offset ceiling), not something `check_market_resolutions`'s presence is currently mitigating. Recommend a **separate** follow-up ticket to (a) backfill `end_date`/`resolution_date` for the no-date segment so it enters passes 2-4's windows, and (b) investigate Gamma's new `offset` ceiling / adopt `/markets/keyset` pagination in `fetch_all_resolved_markets` — neither blocks O-13 removal.
...
0. ~~Do §5b's spot-check first, before deleting anything.~~ **Done 2026-07-01** — see §5b item 1's "CLOSED" addendum. 25-market live sample found 0 dead/stub, 76% already resolved-on-Gamma-but-stuck-locally, 24% genuinely still open; none reachable by pass 1 (no `endDate` field, plus a newly-discovered Gamma offset ceiling far below the assumed 50K); `check_market_resolutions` itself is confirmed mechanistically broken for this segment (hex-id 422s, `payoutNumerator` parsing bug against the current Gamma schema) and provides no working backstop today. **Removal is proven-safe, not just assumed-safe** — proceed with steps 1-5 below with no remaining open item.
