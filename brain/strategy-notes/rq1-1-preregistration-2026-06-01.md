...
|---------|-----------|-------------|
| **STRONG PASS** | r ≤ −0.40 AND p < 0.05 AND n ≥ 30 | ELO system validated — proceed to RQ3.2 and Phase 5 gate |
| **WEAK PASS** | −0.40 < r ≤ −0.25 AND p < 0.05 AND n ≥ 30 | Proceed with caution — do not increase position sizing; monitor for 4 more weeks |
| **INCONCLUSIVE** | −0.25 < r ≤ 0 AND any p, OR p ≥ 0.05 at any r ≤ 0, OR n < 30 | Extend Period 2 to September 1, 2026 — rerun then |
| **DIRECTIONAL FAIL** | r > 0 AND p < 0.05 AND n ≥ 30 | ELO inversely predicts accuracy — trigger stopping-rule review |
| **NOISE FAIL** | r > 0 AND p ≥ 0.05 | Record as inconclusive — weak wrong-direction result, not stopping-rule trigger |

Note on the May 3 document: Oscar's pre-registration listed STRONG PASS as
"r ≥ 0.40." Given the Brier scoring convention (lower = better), the
expected correlation is negative. The thresholds above treat Oscar's 0.40
and 0.25 magnitudes as |r| thresholds with the required negative sign. If
Oscar intended to measure r against inverted Brier (accuracy = 1 − Brier),
the sign would flip to positive — in that case the May 3 thresholds are
literally correct. Either way the magnitude thresholds are the same.
Resolve this ambiguity before the run: confirm sign convention with Oscar
if the April 26 script's convention (r negative = pass) is not authoritative.
